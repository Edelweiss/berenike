...
(SELECT i.loci_site_id as site, concat(IF(i.loci_season_id > 50, '19', '20'), i.loci_season_id) as season, REGEXP_REPLACE(i.loci_trench_id, '^0*(.+)$', '\\1') as trench, CONCAT_WS('', loci_locusno+0, LOWER(REGEXP_SUBSTR(loci_locusno, '[A-Za-z]'))) as locus, REGEXP_REPLACE(i.pb_pb_no, '^0*(.+)$', '\\1') as bucket, count(*) as finds FROM iwona_3 i JOIN import on import.id = i.id and authority = 'iwona_3' GROUP BY site, season, trench, locus, bucket) f
...
WHERE f.site != 'BE' AND f.season is not null AND f.trench is not null AND f.locus is not null AND f.bucket is not null AND b.excavation_id is NOT NULL
